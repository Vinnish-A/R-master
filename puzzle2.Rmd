...
在$\alpha = 0.05$的水准上，拒绝域为0.95，可以用qnorm计算得给定概率0.95后的下分位点，对于标准正态分布来说是`r qnorm(0.95)`，当样本均值$\hat{\mu}$\>$1.644 \times \bar{\sigma}/\sqrt{n}$时，认为可以拒绝$H_0$。

样本均值为`r mean(samples)`，大于0.01644，落在拒绝域

```{r fig.align='center', fig.cap="α值就是黑色虚线后的曲线下面积"}
ggplot(tibble(x = c(-1.2, 1.2)), aes(x = x)) +
  stat_function(fun = dnorm, args = list(mean = 0, sd = 1/10), color = "firebrick") + 
...
根据最大似然，样本均数最有可能服从总体均数为样本均数，标准差为1/10（由于总体方差已知）的正态分布。当$\alpha = 0.05$时，拒绝域分位值为`r qnorm(0.95) * 1/10`。β值根据定义为犯二类错误，也就是假阴性的概率，所以可以为黑色虚线前的曲线下面积。

```{r fig.align='center', fig.cap="β值就是黑色虚线前的曲线下面积"}
